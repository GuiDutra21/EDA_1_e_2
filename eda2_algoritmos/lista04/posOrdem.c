...
{  
    if(raiz == NULL)
        return;
    pilha *cabecaAux = malloc(sizeof(pilha)); // Pilha auxiliar para a navegacao
    pilha *cabeca = malloc(sizeof(pilha)); // Contera os dados na ordem contraria ao pos ordem
    cabecaAux -> prox = NULL;
    cabeca -> prox = NULL;
    no *atual = raiz;
    ...
    do
    {
        ...
        if(atual ->esq != NULL && atual -> dir != NULL)
        {
            ...
            atual = desempilha(cabecaAux);

    } while (atual != NULL);

    while(cabeca -> prox != NULL) // Printa os elementos
    {
